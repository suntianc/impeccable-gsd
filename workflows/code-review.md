...
- gsd-impeccable-reviewer: Reviews frontend/UI code using Impeccable design principles (for UI files)
</available_agent_types>
...
Classify files into frontend and backend categories:

```bash
FRONTEND_EXTENSIONS=".tsx .jsx .vue .svelte .css .scss .sass .less .html .astro"
FRONTEND_FILES=()
BACKEND_FILES=()
...
for file in "${REVIEW_FILES[@]}"; do
  ...
  IS_FRONTEND=false
  for frontend_ext in $FRONTEND_EXTENSIONS; do
    if [[ ".$EXT_LOWER" == "$frontend_ext" ]]; then
      IS_FRONTEND=true
      break
    fi
  done

  # Also check for frontend path patterns
  if [[ "$file" =~ /(components|pages|views|layouts|styles|ui)/ ]]; then
    IS_FRONTEND=true
  fi
...
<step name="check_impeccable_context">
If frontend files are detected, verify .impeccable.md exists:

```bash
if [ "$FRONTEND_COUNT" -gt 0 ]; then
  if [ ! -f .impeccable.md ]; then
    echo "⚠ Frontend files detected but .impeccable.md not found."
    echo "Run '$ gsd-ui-phase ${PHASE_ARG}' to initialize design context first."
    echo "Skipping Impeccable review for frontend files."
    # Continue with backend-only review
    FRONTEND_FILES=()
    FRONTEND_COUNT=0
  else
    echo "✅ Design context found: .impeccable.md"
  fi
fi
```
</step>
...
**Dispatch logic based on file classification:**

**Case 1: Only backend files (FRONTEND_COUNT = 0)**

Spawn gsd-code-reviewer agent normally:

```bash
# Build files_to_read block
FILES_TO_READ=""
...
**Case 2: Only frontend files (BACKEND_COUNT = 0)**

Spawn gsd-impeccable-reviewer agent:
...
🎨 Frontend files detected — delegating to Impeccable reviewer
   Files: ${FRONTEND_COUNT} frontend files
   Design Context: .impeccable.md
```

```
Agent(subagent_type="gsd-impeccable-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
Review the listed frontend source files using Impeccable design principles.
Evaluate typography, color, layout, responsiveness, accessibility, and AI Slop.
Write findings to ${REVIEW_PATH} with design quality scores.
Do NOT commit the output — the orchestrator handles that.
")
```

**Case 3: Mixed frontend and backend files**

Spawn BOTH agents (sequentially, not in parallel):

1. **First: Spawn gsd-impeccable-reviewer for frontend files:**

```bash
FRONTEND_FILES_TO_READ=""
for file in "${FRONTEND_FILES[@]}"; do
  FRONTEND_FILES_TO_READ+="- ${file}\n"
done

FRONTEND_CONFIG_FILES=""
for file in "${FRONTEND_FILES[@]}"; do
  FRONTEND_CONFIG_FILES+="  - ${file}\n"
done

FRONTEND_REVIEW_PATH="${PHASE_DIR}/${PADDED_PHASE}-REVIEW-FRONTEND.md"
```

```
🎨 Frontend files detected — delegating to Impeccable reviewer
   Frontend: ${FRONTEND_COUNT} files
   Backend: ${BACKEND_COUNT} files (will review after)
```
...
depth: ${REVIEW_DEPTH}
phase_dir: ${PHASE_DIR}
review_path: ${FRONTEND_REVIEW_PATH}
files:
${FRONTEND_CONFIG_FILES}
design_context: .impeccable.md
</config>
...
Review the listed frontend source files using Impeccable design principles.
Write findings to ${FRONTEND_REVIEW_PATH} with design quality scores.
Do NOT commit the output — the orchestrator handles that.
")
```

2. **Then: Spawn gsd-code-reviewer for backend files:**

```bash
BACKEND_FILES_TO_READ=""
for file in "${BACKEND_FILES[@]}"; do
  BACKEND_FILES_TO_READ+="- ${file}\n"
done

BACKEND_CONFIG_FILES=""
for file in "${BACKEND_FILES[@]}"; do
  BACKEND_CONFIG_FILES+="  - ${file}\n"
done

BACKEND_REVIEW_PATH="${PHASE_DIR}/${PADDED_PHASE}-REVIEW-BACKEND.md"
```
...
depth: ${REVIEW_DEPTH}
phase_dir: ${PHASE_DIR}
review_path: ${BACKEND_REVIEW_PATH}
${DIFF_BASE:+diff_base: ${DIFF_BASE}}
files:
${BACKEND_CONFIG_FILES}
</config>

Review the listed backend source files at ${REVIEW_DEPTH} depth.
Write findings to ${BACKEND_REVIEW_PATH}.
Do NOT commit the output — the orchestrator handles that.
")
```
...
After both agents complete, merge the reports:

```bash
# Combine both review files
cat "${FRONTEND_REVIEW_PATH}" > "${REVIEW_PATH}"
echo "" >> "${REVIEW_PATH}"
echo "---" >> "${REVIEW_PATH}"
echo "" >> "${REVIEW_PATH}"
cat "${BACKEND_REVIEW_PATH}" >> "${REVIEW_PATH}"

# Clean up temporary files
rm -f "${FRONTEND_REVIEW_PATH}" "${BACKEND_REVIEW_PATH}"
```
...
If any Agent() call fails (agent error, timeout, or exception):
```
