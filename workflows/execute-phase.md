...
- gsd-impeccable-executor — Executes UI/Frontend plans using Impeccable design principles (for UI plans)
...
2.7. **Impeccable delegation (UI plan detection):**

   Before spawning normal executor agents, check if this plan involves UI/Frontend work.
   ...
   **Detection rules:**
   ...
   if plan.type == "ui" or plan.type == "frontend" or plan.type == "design":
     IS_UI_PLAN = true

   # Check plan objective for UI keywords
   UI_KEYWORDS = ["component", "UI", "interface", "layout", "styling", "frontend",
                  "responsive", "animation", "page", "view", "template"]
   if any keyword in plan.objective.lower() for keyword in UI_KEYWORDS:
     IS_UI_PLAN = true

   # Check files_modified for frontend file extensions
   FRONTEND_EXTENSIONS = [".tsx", ".jsx", ".vue", ".svelte", ".css", ".scss", ".sass",
                          ".less", ".html", ".astro"]
   if any file ends with extension for file in plan.files_modified:
     IS_UI_PLAN = true

   # Check plan tags
   if plan has tag "[UI]" or "[Frontend]" or "[Design]":
     IS_UI_PLAN = true
   ```

   **If `IS_UI_PLAN` is true:**

   1. **Verify .impeccable.md exists:**
      ```bash
      if [ ! -f .impeccable.md ]; then
        echo "⚠ UI plan detected but .impeccable.md not found."
        echo "Run '$ gsd-ui-phase {PHASE_NUMBER}' to initialize design context first."
        exit 1
      fi
      ```
   ...
   2. **Log the delegation:**
      ```
      🎨 Plan {plan_id} identified as UI work — delegating to Impeccable executor
         Type: {plan.type}
         Files: {frontend files from files_modified}
         Objective: {first 50 chars of plan objective}
      ```

   3. **Spawn gsd-impeccable-executor agent** (instead of gsd-executor):

      ```text
      Agent(
        subagent_type="gsd-impeccable-executor",
        description="Impeccable UI execution for plan {plan_number} of phase {phase_number}",
        ...
        prompt="
          <objective>
          Execute UI plan {plan_number} of phase {phase_number}-{phase_name} using Impeccable design principles.
          Commit each task atomically. Create SUMMARY.md with design quality report.
          ...
          ```
          Per-commit HEAD/cwd-drift/path-guard: `agents/gsd-impeccable-executor.md` + `references/worktree-path-safety.md` (in <execution_context>).
          </worktree_branch_check>
          ...
          <execution_context>
          @$HOME/.claude/agents/gsd-impeccable-executor.md
          ...
          - .impeccable.md (Design context — REQUIRED)
          - .planning/PROJECT.md (Project context)
          - .planning/config.json (Config, if exists)
          - ./CLAUDE.md (Project instructions, if exists)
          </files_to_read>

          <impeccable_references>
          Load relevant Impeccable references based on plan requirements:
          ...
          @$HOME/.claude/skills/impeccable/references/responsive-design.md
          @$HOME/.claude/skills/impeccable/references/interaction-design.md
          </impeccable_references>

          <success_criteria>
          - [ ] All tasks executed following Impeccable principles
          - [ ] Each task committed individually
          - [ ] SUMMARY.md created with design quality report
   ...
   4. **After Impeccable executor returns:**
      - Read the SUMMARY.md and verify it contains design quality report
      - Log the handoff:
        ```
        ✅ Impeccable execution complete for plan {plan_id}
   ...
   **If `IS_UI_PLAN` is false:** Continue to normal executor dispatch below.
